...
def elmejormaestrocuri():
    ...
    # Ciclos que genera las combinaciones de valores booleanos
    for w in range(2):
        for x in range(2):
            for y in range(2):
                for z in range(2):
    ...
    for n in range(16):
        ...
        print("[%d][%d][%d][%d] = %d" % (m[n][0], m[n][1], m[n][2], m[n][3], result)) #Imprimir resultado en el orden dado
    
    #Ciclo que genera la función Booleana disyuntiva
    print("--------------Funcion Booleana Disyuntiva-------------------")
    for n in range(16):
        if f(m[n][0], m[n][1], m[n][2], m[n][3]) == 1: #Condicional si el valor de la tabla es igual a 1
            print("( ", end="")
            if m[n][0] == 0:
                print("w'", end=" ")
            if m[n][0] == 1:
    ...
    #Ciclo que genera la función Booleana conjuntiva
